...
public class Salesman extends Person {
    ...
    public void whatSalesmanOffer() {

        StringBuilder builder = new StringBuilder("Продавец "+getName()+" "+ getSecondName()+" предлагает:\n");

        if (products.size() == 0) {
            builder.append("ничего \n");
        } else {
            for (Product offer : products) {
                builder.append(offer.getName());
                builder.append(" в количестве ");
                builder.append(offer.getCount());
                builder.append(" ");
                builder.append (" Цена "+offer.getPrice());
                builder.append("\n");
            }
        }

        builder.append("У него есть " + this.getCash());
